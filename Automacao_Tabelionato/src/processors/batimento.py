...
class TabelionatoBatimento:
    ...
    def _formatar_layout_saida(self, df: pd.DataFrame) -> pd.DataFrame:
        """Mapeia colunas para o layout final esperado pelo sistema de importaÃ§Ã£o."""
        ...
        if ausentes:
            ...
            raise KeyError(
                "Coluna obrigatÃ³ria ausente no batimento. "
                f"Faltando: {ausentes}. DisponÃ­veis: [{colunas_disponiveis}]"
            )

        # Criar DataFrame de saÃ­da com ordem especÃ­fica das colunas
        ...
        # Adicionar Campanha no inÃ­cio se existir
