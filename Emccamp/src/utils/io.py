...
def write_csv_to_zip(
    ...
    # BOM para UTF-8 (permite que Excel reconheça o encoding)
    UTF8_BOM = b'\xef\xbb\xbf'

    with zipfile.ZipFile(zip_path, 'w', compression=zipfile.ZIP_DEFLATED) as zf:
        for name, df in dataframes.items():
            # Escrever CSV para string
            csv_content = df.to_csv(index=False, sep=sep, decimal=',')
            # Encode para UTF-8 com BOM
